...
@implementation DragFileInEditText

-(instancetype)init{
    self = [super init];
    if(self){
        [self initDragFileEvent];
    }
    return self;
}
...
-(void)initDragFileEvent{
    ...
    [self registerForDraggedTypes:[NSArray arrayWithObjects:NSFilenamesPboardType, nil]]; // NSFilenamesPboardType
...
-(BOOL)prepareForDragOperation:(id<NSDraggingInfo>)sender{
    ...
    // 3）、将接受到的文件链接数组通过代理传送
    if(list.count>0 && self.dragDelegate && [self.dragDelegate respondsToSelector:@selector(onDragFileInEditTextDragInFile:withTitle:)]){
        [self.dragDelegate onDragFileInEditTextDragInFile:list[0] withTitle:self.shaderType];
    }
    return YES;
}
