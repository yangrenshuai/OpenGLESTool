...
@implementation DragFileInButton
...
-(BOOL)prepareForDragOperation:(id<NSDraggingInfo>)sender{
    ...
    // 3）、将接受到的文件链接数组通过代理传送
    if(list.count>0 && self.delegate && [self.delegate respondsToSelector:@selector(onDragFileInButtonDragInFile:withTag:)]){
        [self.delegate onDragFileInButtonDragInFile:list[0] withTag:self.tag];
    }
    return YES;
}
